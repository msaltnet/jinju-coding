...
스위치와 피에조 부저에 대해서 알아보고, 스위치 버튼을 눌러서 피에조 부저를 통해 소리를 내는 회로와 프로그램을 만들어 봅니다.
...
다음으로 피에조 부저를 브레드보드 왼쪽 끝에 꽂아주세요. +가 왼쪽으로 오게 꽂으면 됩니다. 비스듬히 꽂아도 됩니다.
...
다음은 저항을 연결해 주겠습니다. 저항은 스위치의 다리 한 쪽과 브레드보드의 `-선`과 연결해주세요. 부저는 저항을 연결하지 않아도 됩니다.
...
이제 버튼을 누르면 버저에서 소리가 나는 프로그램을 작성해보겠습니다. 만들어진 프로그램을 파일로 불러와서 아두이노 IDE에서 아두이노로 업로드 해보세요.
